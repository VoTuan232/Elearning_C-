...
namespace ELearning.Controllers
{
    public class CourseController : Controller
    {
        ...
        public ActionResult Index(int? page)
        {
            ...
            {
                var related = unitofwork.Courses.GetAll().Where(x => x.Status).OrderByDescending(x => x.ModifiedDate).Take(3).ToList();
                ViewBag.Related = related;

                var items = unitofwork.Courses.GetAll().Where(x => x.Status).Select(x => new CourseDto
                {
                    Id = x.Id,
                    Name = x.Name,
                    Image = x.Image,
        ...
        public ActionResult Search(string keyword, int? page)
        {
            ...
            using (var workScope = new UnitOfWork(new ELearningDBContext()))
            {
                ...
                var related = workScope.Courses.GetAll().Where(x => x.Status).OrderByDescending(x => x.ModifiedDate).Take(3).ToList();
                ViewBag.Related = related;
